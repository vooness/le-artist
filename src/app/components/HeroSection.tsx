...
// Dynamický import ParticlesBackground pro lepší výkon
const ParticlesBackground = dynamic(() => import('./ParticlesBakckground'), {
  ssr: false,
  loading: () => null
})
...
export const HeroSection: React.FC = () => {
  ...
  // Detekce client-side renderování
  const [isClient, setIsClient] = useState(false);

  useEffect(() => {
    // Označení, že jsme na klientovi
    setIsClient(true);
    
    // Detekce mobilního zařízení na základě šířky obrazovky
    const checkMobile = () => {
  ...
  // Pomocné funkce pro renderování s/bez animací
  const renderWithAnimation = (element, animation) => {
    if (!isClient || isMobile) return element;
    return animation;
  }
  ...
  return (
    <section className="relative bg-[#0c1320] text-white py-12 sm:py-20 lg:py-28 flex flex-col items-center justify-center overflow-hidden px-6 sm:px-8 lg:px-20 min-h-screen">
      {/* Zobrazit ParticlesBackground pouze na desktopu */}
      ...
      <div className="absolute inset-0 z-0">
        ...
        {/* Animované tečky - pouze na desktopu */}
        {isClient && !isMobile && (
        ...
        {/* Horizontální čáry - pouze na desktopu */}
        {isClient && !isMobile && (
          <>
            {[...Array(3)].map((_, i) => (
              <motion.div 
        ...
        {/* Statické horizontální čáry pro mobil */}
        {isClient && isMobile && (
          <>
            {[...Array(3)].map((_, i) => (
              <div 
      ...
      <div className="relative flex flex-col items-center w-full max-w-7xl mt-12 z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center w-full flex-wrap">
          ...
          {renderWithAnimation(
            <div className="flex flex-col gap-4 sm:gap-6 text-center lg:text-left flex-1">
              ...
              <div className="relative">
                <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight">
                  ...
                  <div className="inline-block relative">
                    ...
                    <div className="absolute left-0 bottom-[-5px] h-1 w-full bg-gradient-to-r from-orange-500 to-yellow-500 rounded-full origin-center"></div>
                  </div>
                </h1>
                
                {!isMobile && (
                  <>
                    <div className="absolute -top-4 -left-4 w-8 h-8 border-t-2 border-l-2 border-orange-500/30 hidden lg:block"></div>
                    <div className="absolute -bottom-4 -right-4 w-8 h-8 border-b-2 border-r-2 border-orange-500/30 hidden lg:block"></div>
                  </>
                )}
              </div>
              ...
              <div className="flex gap-4 mt-6 justify-center lg:justify-start">
                <a 
                  href="/sluzby" 
                  className="relative group px-6 py-3 bg-orange-500 text-white font-medium rounded-full overflow-hidden hover:scale-105 active:scale-95 transition-transform"
                >
          ...
          {/* Pravá část s ilustrací - zobrazit pouze na desktopu */}
          {isClient && !isMobile && (
          ...
          <div className="grid grid-cols-2 gap-6 text-center mt-12 lg:mt-36 xl:grid-cols-4 w-full max-w-5xl">
            {[
              ...
              <div key={index} className="relative group">
                <div className="absolute inset-0 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-lg border border-white/5 -z-10 transition-all duration-300 hover:shadow-[0_0_15px_2px_rgba(249,115,22,0.3)]" />
